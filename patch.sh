...
###############################################################################
# 1) Ensure ribbon header styles (matches the tabs tint)
###############################################################################
...
.section-header{
  background:#eef2ff;              /* indigo-100 ribbon */
...
###############################################################################
# 2) header.html:
#    - Layout Diagram -> ribbon header
#    - Job Dashboard  -> ribbon header (remove old header+button row)
#    - Remove ALL existing toolbars with dash* buttons
#    - Insert one compact bottom toolbar after #dbAlerts
###############################################################################

# 2a) Layout Diagram: <h2> -> ribbon header
perl -0777 -i -pe '
  s|<h2[^>]*>\s*Layout\s+Diagram\s*</h2>|<div class="section-header"><span class="section-title">Layout Diagram</span></div>|s
' "$HDR"

# 2b) Job Dashboard: remove the top header+buttons row and replace with ribbon
perl -0777 -i -pe '
...
# 2c) Remove ANY existing dashboard toolbars (defensive dedupe)
#     (Any <div> that contains a dash* button gets removed.)
perl -0777 -i -pe '
  s|\n\s*<div[^>]*>\s*(?:(?!</div>).)*(id="dashSave"|id="dashBook"|id="dashUndo"|id="dashQuote")(?:(?!.*/div>).)*</div>\s*||gs
' "$HDR"

# 2d) Insert ONE compact toolbar after the alerts div in the dashboard
perl -0777 -i -pe '
...
echo "==> Done. Job Dashboard uses ribbon header; single bottom toolbar."
